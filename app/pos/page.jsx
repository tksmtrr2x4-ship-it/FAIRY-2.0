    ...
    return (
      <div className="min-h-screen bg-[#F5F5F7] flex flex-col items-center justify-center font-sans antialiased text-[#1D1D1F] p-6">
        <div className="max-w-md w-full text-center bg-white/70 backdrop-blur-xl border border-white/20 p-10 rounded-3xl shadow-xl flex flex-col items-center">
          ...
          <h2 className="text-2xl font-extrabold text-[#0B2F5C] tracking-tight">Systemaktualisierung</h2>
    ...
    <div className="min-h-screen bg-[#F5F5F7] text-[#1D1D1F] font-sans antialiased flex flex-col selection:bg-[#0B2F5C] selection:text-white">
      ...
      <header className="sticky top-0 z-40 backdrop-blur-md bg-white/75 border-b border-gray-200/50 px-8 py-4 flex justify-between items-center">
        <div className="flex items-center gap-6">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-[#0B2F5C]">Weltladen St. Ursula</h1>
            <p className="text-xs text-gray-400 font-bold tracking-wide">FAIRTRADE SCHÜLERFIRMA • VILLINGEN</p>
          </div>
          <div className="h-6 w-px bg-gray-200" />
          <div className="text-sm font-semibold text-[#F2B600] bg-[#F2B600]/10 px-3 py-1 rounded-full flex items-center gap-2">
            <span className="h-2 w-2 rounded-full bg-[#F2B600] animate-pulse" />
            Live-Kasse
          </div>
        </div>
      ...
      {config.bannerActive && (
        <div className="bg-[#F2B600]/10 border-b border-[#F2B600]/20 text-[#0B2F5C] py-3 px-8 text-center text-sm font-bold tracking-wide flex items-center justify-center gap-2 animate-pulse">
          <span className="h-2 w-2 rounded-full bg-[#F2B600]" />
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 p-8">
        <main className="col-span-8 flex flex-col gap-6">
          ...
          {products.length === 0 ? (
            <div className="flex-1 bg-white border border-gray-100 rounded-3xl p-12 flex flex-col items-center justify-center text-center">
              <h3 className="text-lg font-bold text-[#0B2F5C]">Lade Produkte...</h3>
            </div>
          ) : (
              ...
              {products
                ...
                .map(product => (
                  <button
                    ...
                    className="p-5 bg-white border border-gray-100/80 hover:border-[#0B2F5C] rounded-3xl shadow-sm hover:shadow-xl text-left flex flex-col justify-between h-36 transition-all duration-300 hover:-translate-y-0.5 active:scale-95 group"
                  >
                    <div>
                      <span className="text-[10px] font-bold text-[#F2B600] tracking-widest uppercase bg-[#F2B600]/5 px-2 py-0.5 rounded-full">{product.group}</span>
                      <h3 className="text-sm font-bold text-[#1D1D1F] line-clamp-2 mt-2 group-hover:text-[#0B2F5C] transition-colors">{product.name}</h3>
                    </div>
                    <span className="text-base font-bold text-[#0B2F5C] mt-4">
        ...
        <aside className="col-span-4 bg-white border border-gray-200/50 rounded-3xl p-6 flex flex-col justify-between shadow-xl backdrop-blur-lg">
          <div>
            <h2 className="text-lg font-bold border-b pb-4 mb-4 text-[#0B2F5C] tracking-tight">Einkaufszettel</h2>
          ...
          <div className="border-t pt-4">
            <div className="flex justify-between text-lg font-bold mb-6">
              ...
              <span className="text-2xl font-bold text-[#0B2F5C]">{totalCartPrice.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}</span>
            </div>
            <button 
              onClick={handleCheckout} 
              disabled={cart.length === 0} 
              className="w-full py-4 bg-[#0B2F5C] hover:bg-[#153e63] disabled:bg-gray-100 disabled:text-gray-300 text-white font-bold rounded-2xl transition-all duration-300 shadow-md active:scale-95"
            >
              Kauf abschließen
            </button>
            {lastSaleId && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-md p-4">
          <div className="bg-white/95 max-w-md w-full rounded-3xl p-8 shadow-2xl border border-white/20 relative">
            ...
            <div className="flex flex-col gap-4 font-mono text-sm text-gray-700">
              ...
              <div className="flex justify-between border-t-2 border-dashed border-gray-300 pt-4 text-base font-bold text-[#0B2F5C] font-sans"><span>Soll-Bargeld:</span><span>{reportData.brutto.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}</span></div>
            </div>
            <div className="mt-8 border-t pt-6 flex flex-col gap-3 font-sans">
              <button onClick={confirmReport} className="w-full py-4 bg-[#0B2F5C] hover:bg-[#163f61] text-white font-bold rounded-2xl shadow-md">Bericht archivieren</button>
